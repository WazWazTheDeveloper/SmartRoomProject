...
export const httpRequestLogger = (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', function () {
        ...
        loggerRequest.info(`${req.method} ${req.url} ${req.headers.origin} status:${res.statusCode} response time:${duration}ms`)
        console.log(duration)
    });

    res.on('closed', function () {
        const duration = (getTime(new Date()) - start);
        loggerRequest.info(`${req.method} ${req.url} ${req.headers.origin} [connection closed]`)
        console.log(duration)
    });
